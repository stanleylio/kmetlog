...
def taskDAQ():
    ...
    try:
        ...
        send({'tag':'Rotronics',
              'ts':ts,
              'Rotronics_T':rhv[config.DAQ_CH_MAP['Rotronics_T']]*100 - 30, # from Volt to Deg.C
              'Rotronics_RH':rhv[config.DAQ_CH_MAP['Rotronics_RH']]*100})   # %RH

        send({'tag':'RMYRTD',
              'ts':ts,
              'RMYRTD_T':rhv[config.DAQ_CH_MAP['RMYRTD_T']]*100 - 50})  # [0,1] V maps to [-50,50] DegC
