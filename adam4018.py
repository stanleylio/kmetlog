# Functions to handle communication with the ADAM4018 Data Acquisition Module (DAQ)
#
# Stanley H.I. Lio. hlio@hawaii.edu.
# OTG - All Rights Reserved, 2016
import serial
import io
import time
import logging
...
class ADAM4018(object):
    ...
    def __init__(self,address,port,baud=9600):
        ...
        logging.debug('ADAM4018 constructor exits')
    ...
    def SetInputRange(self,InputRange):
        logging.debug('SetInputRange(), from {} to {}'.format(self._currentinputrange,InputRange))
        ...
        if not InputRange == self.GetInputRange():
            if self._configuration(InputRange=InputRange):
                ...
                logging.info('SetInputRange() changed to {}'.format(InputRange))
                return True
            else:
                logging.warning('_configuration(InputRange={}) failed'.format(InputRange))
                return False
        else:
            logging.debug('SetInputRange(): No change needed.')
            return True

    def SetInputRangeAuto(self,v):
        """Given a voltage v, set the DAQ to the best InputRange"""
        assert type(v) is float
        logging.debug('SetInputRangeAuto() for {}'.format(v))
        vs = sorted(self._ranges.keys())
        if v > max(vs):
            logging.error('SetInputRangeAuto(): the specified v={} is out of range'.format(v))
            return False
        for vr in vs:
            if vr >= v:
                ...
                if not r:
                    print 'ayu'
                return r
        logging.warning('SetInputRangeAuto(): the supplied v is too large. Setting input range to maximum {}.'.format(max(vs)))
    ...
    def GetInputRange(self):
        ...
        for k in self._ranges:
            if self._ranges[k] == r[3:5]:
                ...
                logging.debug('GetInputRange(): current = {}'.format(k))
                return k
        logging.warning('GetInputRange() failed: {}'.format(r))
        return None

    def ReadChannel(self,channel):
        logging.debug('ReadChannel()')
    ...
    def ReadChannelAutoRange(self,channel):
        ...
        logging.debug('ReadChannelAutoRange()')
        ...
        if vr == self._currentinputrange and v < self._currentinputrange:
            logging.debug('ReadChannelAutoRange() early exit')
            #return v,self._currentinputrange
            return v
        
        #vs = sorted(self._ranges.keys())
        if self._currentinputrange != max(vs):
            #logging.debug('setting new input range...')
            irset = False
            for i in range(3):
                if self.SetInputRange(max(vs)):
                    ...
                    break
            if not irset:
                logging.warning('1st SetInputRange() failed')
                return None
        v = self.ReadChannel(channel)
        if v > max(vs):
            logging.error('Invariant violation: ReadChannel() = {} > {}'.format(v,max(vs)))
            return None
        if self.SetInputRangeAuto(v):
            #return self.ReadChannel(channel),vr
            return self.ReadChannel(channel)
        logging.error('wut? {},{}'.format(v,self._currentinputrange))
        return None
        ...
        if r.startswith('>'):
            ...
            if 1+7*8 == len(r): # a '>' plus eight 7-chr fields
                try:
                    ...
                    logging.error('wut? {}'.format(str(r)))
        return None
...
FCI: Format, Checksum and Integration time. P.117"""
        ...
        if 10 == len(r) and '!' == r[0] and '\r' == r[-1] and self._address == r[1:3]:
            ...
            if r.startswith('!') and self._address == r[1:3]:
                ...
                time.sleep(7)
                return True
            logging.warning('_configuration(): {}'.format(r))
            return False
        elif 4 == len(r) and '?' == r[0] and '\r' == r[-1] and self._address == r[1:3]:
            logging.warning('Invalid command. Response={}'.format(r))    # P.121
        else:
            logging.debug('Unexpected response from serial bus: {}'.format(r))
            return None
